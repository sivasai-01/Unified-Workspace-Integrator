...
def fetch_items(
    ...
    if response.status_code == 200:
        ...
        print("Airtable API error:", response.status_code, response.text)
...
async def get_items_airtable(credentials) -> list[dict]:
    ...
    items = []
    list_of_responses = []
    print("credentials", credentials.get('access_token'))
    fetch_items(credentials.get('access_token'), url, list_of_responses)
    print("List of responses fetched", list_of_responses)
    for response in list_of_responses:
        # Add base item
        base_item = create_integration_item_metadata_object(response, 'Base')
        items.append(base_item.__dict__)
        ...
        if tables_response.status_code == 200:
            ...
            for table in tables_response['tables']:
                ...
                items.append(table_item.__dict__)
